...
RegisterCommand('transfervehicle', function(source, args)
	...
	if(GetPlayerName(tonumber(args[1])))then
		   ...
		   TriggerClientEvent('chat:addMessage', source, { 
            ...
            args = { 'SYSTEM', "Incorrect player ID!"}
        })
			return
	end
	...
	MySQL.Async.fetchAll(
        ...
        function(result)
            if result[1] ~= nil then
				...
				if myID == CarOwner then
					...
					data = {}
					TriggerClientEvent('cuta-tasknotify:client:SendAlert', -1, { type = 'inform', text = "" .. plate .." - OWNED BY " .. myName .. " SOLD TO " .. targetSteamName}, 19000)
				--TriggerClientEvent('cuta-tasknotify:client:SendAlert', -1, { type = 'inform', text = 'PLAT ' .. plate ..' - MILIK ' ..myName .. ' TERJUAL KE ' .. targetSteamName, length = 19000, style = { ['background-color'] = '#ffffff', ['color'] = '#000000' } })
					...
					print("GA KE TRANSFER SAYANG")
					--TriggerClientEvent('chatMessage', source, "^*^1You do not own the vehicle")
					TriggerClientEvent('chat:addMessage', source, { 
                        template = '<div class="chat-message emergency"><b>{0}</b>: {1}</div>',
                        args = { 'SYSTEM', "You do not own the vehicle"}
                    })
				end
			else
		...
		TriggerClientEvent('chat:addMessage', source, { 
			template = '<div class="chat-message emergency"><b>{0}</b>: {1}</div>',
			args = { 'SYSTEM', "ERROR : This vehicle plate does not exist or the plate was incorrectly written."}
		})
            end
